...
def get_keyboard():
    buttons = [types.InlineKeyboardButton(text="Далее", callback_data="sedentary_labor_cards_state_1.1"),
               types.InlineKeyboardButton(text="Назад", callback_data="sedentary_labor_cards_state_1.2"),
...
def get_end_keyboard():
    buttons = [types.InlineKeyboardButton(text="Разумеется", callback_data="sedentary_labor_cards_end_state_1.1"),
               types.InlineKeyboardButton(text="Назад", callback_data="sedentary_labor_cards_end_state_1.2"),
...
async def start_sedentary_labor_cards(call):
    number_exercise = 1
    cursor = connection.cursor()
    change_number_exercise_from_db(cursor, number_exercise, call.from_user.id)
    card = Card(number_exercise, 'sedentary_monotonous_work')
...
@dp.callback_query_handler(Text(startswith="sedentary_labor_cards_end_state_1"))
...
@dp.callback_query_handler(Text(startswith="sedentary_labor_cards_state_1"))
async def callbacks_num(call: types.CallbackQuery):
    action = call.data.split(".")[1]
    print('dsgi')
    cursor = connection.cursor()
    ...
    number_exercise = number_exercise[0]
    cursor.execute("SELECT COUNT(*) from sedentary_monotonous_work")
