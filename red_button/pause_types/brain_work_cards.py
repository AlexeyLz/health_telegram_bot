...
def get_keyboard():
    buttons = [types.InlineKeyboardButton(text="Далее", callback_data="brainwork1_cards_state_1.1"),
               types.InlineKeyboardButton(text="Назад", callback_data="brainwork1_cards_state_1.2"),
...
def get_end_keyboard():
    buttons = [types.InlineKeyboardButton(text="Разумеется", callback_data="brainwork_cards_end_state_1.1"),
               types.InlineKeyboardButton(text="Назад", callback_data="brainwork_cards_end_state_1.2"),
...
@dp.callback_query_handler(Text(startswith="brainwork_cards_end_state_1"))
...
@dp.callback_query_handler(Text(startswith="brainwork1_cards_state_1"))
async def callbacks_num(call: types.CallbackQuery):
    action = call.data.split(".")[1]
    print('dsgi')
    cursor = connection.cursor()
    ...
    number_exercise = number_exercise[0]
    cursor.execute("SELECT COUNT(*) from brainwork")
